...
namespace Engine3.Test.Voxel.Graphics {
	public unsafe class XyzGizmoRenderingImpl {
		...
		public XyzGizmoRenderingImpl(LogicalGpu logicalGpu, TransferCommandPool transferCommandPool, VkFormat swapChainImageFormat, Assembly gameAssembly, byte maxFramesInFlight) {
			gizmoGraphicsPipeline = CreatePipeline(logicalGpu, swapChainImageFormat, gameAssembly, out DescriptorSetLayout sceneLayout, out DescriptorSetLayout materialLayout);
		...
		public void RecordCommandBuffer(GraphicsCommandBuffer graphicsCommandBuffer, byte frameIndex, VkExtent2D extent) { // TODO see if i can draw with lines instead
			...
			graphicsCommandBuffer.CmdClearDepth(extent);
		...
		public void Copy(Matrix4x4 projectionMatrix, Quaternion cameraOrientation, byte frameIndex) {
			sceneUniformData = new(projectionMatrix, Matrix4x4.Transform(Matrix4x4.Identity, cameraOrientation) * GizmoTransform);
